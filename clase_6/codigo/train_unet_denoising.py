...
output_dir = directorio_script / "resultados" / "unet_denoising"
...
class Unet(nn.Module):
    def __init__(self):
        ...
        self.Up4 = nn.ConvTranspose2d(512, 512, kernel_size=4, stride=2)
        ...
        self.Up3 = nn.ConvTranspose2d(256, 256, kernel_size=4, stride=2)
        ...
        self.Up1 = nn.ConvTranspose2d(64, 64, kernel_size=4, stride=2)
    ...
    def forward(self, x):
        ...
        x = self.middle(self.maxpool(x4))
        x = self.Up4(x)
        x = self.Up4conv(torch.cat([x, x4], dim=1))
        x = self.Up3(x)
        x = self.Up3conv(torch.cat([x, x3], dim=1))
        x = self.Up2(x)
        x = self.Up2conv(torch.cat([x, x2], dim=1))
        x = self.Up1(x)
        x = self.Up1conv(torch.cat([x, x1], dim=1))
        return self.out(x)


Unet()(torch.randn(1, 3, size, size)).shape
